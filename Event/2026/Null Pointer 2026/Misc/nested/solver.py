    ...
    while True:
        ...
        # Handle zip files
            ...
            with zipfile.ZipFile(current_file, 'r') as zip_ref:
                # Check for encrypted files just in case
                members = zip_ref.infolist()
                if members:
                    if members[0].flag_bits & 0x1:
                        print(f"Oh my bad! {current_file} is password protected. We need to crack it.")
                        break
                    extracted_file = members[0].filename
                    zip_ref.extractall()
        ...
        if extracted_file:
            # Clean up file sebelumnya agar tidak menumpuk, kecuali file awal
...
if __name__ == "__main__":
    # Mulai dari layer_1000.zip
    peel_the_onion("layer_1000.zip")
